...
export default function EditTestPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/tests/${id}`)
      ...
      .then((d) => {
        ...
        setSkillCategory(d.skill_category ?? '')
        setPassingScore(d.passing_score ?? 70)
        setTimeLimit(d.time_limit_minutes ?? 30)
  ...
  const updateQuestion = (qid: string, field: keyof Question, value: unknown) => {
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-black text-white">Edit Test</h1>
      </div>
          ...
          {questions.map((q, qIdx) => (
            <div key={q.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
              <div className="flex items-start justify-between gap-3 mb-4">
                ...
                {questions.length > 1 && (
                  <button type="button" onClick={() => removeQuestion(q.id)} className="text-slate-600 hover:text-red-400 transition-colors">
                  ...
                  <div key={i} className="flex items-center gap-3">
                    <button
                      ...
                      className={`flex-shrink-0 w-6 h-6 rounded-full border-2 transition-colors flex items-center justify-center ${
                        q.correct === i ? 'border-emerald-500 bg-emerald-500' : 'border-slate-600 hover:border-slate-500'
                      }`}
                    >
                      {q.correct === i && (
